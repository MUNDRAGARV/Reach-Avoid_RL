...
parser.add_argument(
    "-mu", "--maxUpdates", help="maximal #gradient updates", default=500000,
    type=int
)
parser.add_argument(
...
parser.add_argument(
    "--start_config", type=int, default=0, help="Config index to start from"
)
...
# == CONFIGURATION ==
    ...
    outFolder = os.path.join(args.outFolder, 'car-pe_withoutshield_final_allconfigs-DDQN', name)
    print(outFolder)
    ...
    env.set_constraint(radius=args.consRadius, car='both')
    env.set_target(radius=args.targetRadius, car='evader')  
