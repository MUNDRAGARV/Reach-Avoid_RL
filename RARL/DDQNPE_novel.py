...
class DDQNPursuitEvasion(DDQN):
  ...
  def initBuffer(self, env):
    ...
    while len(self.memory) < self.memory.capacity:
      ...
      s = env.reset()
      actionIdx, actionIdxTuple, _ = self.select_action(s, explore=True)
      s_, r, done, info = env.step(actionIdx)
  ...
  def learn(
      self, env, MAX_UPDATES=10000, MAX_EP_STEPS=100, warmupBuffer=True,
    ...
    while self.cntUpdate <= MAX_UPDATES:
      ...
      for step_num in range(MAX_EP_STEPS):
        # Select action
        # actionIdx, actionIdxTuple, shield_used = self.select_action(s, explore=True)

        # Interact with env
        # s_, r, done, info = env.step(actionIdxTuple)
        # epCost += r
        raw_actionIdx, raw_actionTuple, shield_used = self.select_action(
            s,
            explore=True
        )

        actionIdx = raw_actionIdx
        actionIdxTuple = raw_actionTuple
  ...
  def select_action(self, state, explore=False):
    ...
    if (np.random.rand() < self.EPSILON) and explore:
            rowIdx = np.random.randint(0, self.numActionList[0])
            colIdx = np.random.randint(0, self.numActionList[1])
        ...
        state_action_values = self.Q_network(state_tensor)

        Q_mtx = (
            state_action_values.detach().cpu().reshape(
                self.numActionList[0],
                self.numActionList[1]
            )
        )
    ...
    return actionIdx, actionIdxTuple, shield_used
