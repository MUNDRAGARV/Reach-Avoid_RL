...
class DubinsCarPEEnv_novel(gym.Env):
  """
  ...
  def step(self, action):
    ...
    return np.copy(self.state).astype(np.float32), cost, done, info


  def simulate_step(self, state, evader_action):
        """
        One-step lookahead for safety shield.
        Uses worst-case pursuer action.
        """

        state_evader = np.copy(state[:3])
        state_pursuer = np.copy(state[3:])

        best_value = np.inf
        best_next_state = None

        for pursuer_action in range(self.numActionList[1]):

            u_evader = self.evader.discrete_controls[evader_action]
            u_pursuer = self.pursuer.discrete_controls[pursuer_action]

            next_evader = self.evader.integrate_forward(state_evader, u_evader)
            next_pursuer = self.pursuer.integrate_forward(state_pursuer, u_pursuer)

            next_state = np.concatenate((next_evader, next_pursuer), axis=0)

            value = self.safety_margin(next_state)   # keep consistent
